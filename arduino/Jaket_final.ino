...
Project Title: Toasty Jaket
Project Date: Spring 2023
Project Group #: 22
Project Members: Kamron Wright, Kenneth Regalado, Lazaro Ramos
Program Description: This program will place the Jaket in auto mode as was
planned in our design. The user can optionally adjust the range that enables
each power mode through a Bluetooth enabled application on their smartphone.
...
#define sensorOut A1 // Temperature sensor pin.
...
int PWM_off = 0;
int PWM_high = 236; // 10.40 Volts, ~25 Watts.
int PWM_medium = 184; // 8.05 Volts, ~15 Watts.
int PWM_low = 108; // 4.66 Volts, ~5 Watts.
...
void loop() {
  ...
  float sensorOutVoltage = readSensorOut * aref_voltage;
